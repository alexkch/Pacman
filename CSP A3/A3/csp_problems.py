...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    var_array = []
    full_dom = [1, 2, 3, 4, 5, 6, 7, 8, 9]
    i = 0
    for rows in initial_sudoku_board:
        var_array.append([])
        j = 0
        for c in rows:
            #print str(col) + '\n'
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraints = "Row {}".format(str(var_array.index(row)))
            constraint_list.append(AllDiffConstraint(constraints, row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraints = "Column {}".format(str(colj)) 
            constraint_list.append(AllDiffConstraint(constraints, scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraints = "SubSquare {} {}".format(str(i),str(j))
                constraint_list.append(AllDiffConstraint(constraints, scope))
