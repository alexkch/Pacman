    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        for q_i in qi.domain():
            for q_j in qj.domain():
                if (q_i != q_j):
                	if (abs(q_i - q_j) != abs(i - j)):
                		sat_assignments.append([q_i, q_j])
                		sat_assignments.append([q_j, q_i])

        TableConstraint.__init__(self, name, [qi, qj], sat_assignments)
...
class NValuesConstraint(Constraint):
    	...
    	if self.numUnassigned() > 0:
        	...
        	for var in self.scope():
        		if var.getValue() == self._required:
        			required_var.append(var)

	        if self._lb <= len(required_var) <= self._ub:
	        	return True
	        else:
	        	return False
    ...
    def hasSupport(self, var, val):
        ...
        def NValuesVals(l):
            ...
            for val in l:
            	# print val
                if val[1] == self._required:
                    required_var.append(val[1])
            if self._lb <= len(required_var) <= self._ub:
            	return True
            else:
            	return False
        ...
        if (findvals(varsToAssign, [(var, val)], NValuesVals, UBNValuesVals)):
        	return True
        else:
        	return False